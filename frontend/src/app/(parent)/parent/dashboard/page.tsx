  ...
  useEffect(() => {
    ...
    const loadParentName = async () => {
      try {
        ...
        setParentName(
          resolvedName && resolvedName.length > 0 ? resolvedName : "Parent",
        );
      } catch {
        if (isMounted) {
          setParentName("Parent");
  ...
  const onSubmit = async (data: ChildFormValues) => {
    ...
    try {
      ...
      if (error instanceof ApiRequestError) {
        ...
        if (error.status === 400) {
          showFeedback(
            error.message || "Validation failed. Check input values.",
            "error",
          );
          return;
        }
      ...
      <Stack spacing={2}>
            ...
            {children.map((child) => (
              ...
              >
                <Typography>
                  ...
                  {child.birthDate ?? "Not set"} ({getAgeLabel(child.birthDate)}
                  )
                  {child.group
        ...
        <Paper variant="outlined" sx={{ p: 2.5 }}>
          <Stack spacing={2}>
            ...
            {journalError ? (
              <Typography color="error.main">{journalError}</Typography>
            ) : null}
            ...
            journalEntries.length === 0 ? (
              <Typography color="text.secondary">
                No journal entries yet. New updates from teachers will appear
                here.
              </Typography>
            ) : null}
