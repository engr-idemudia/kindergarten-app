...
export default function ParentChildrenPage() {
  ...
  useEffect(() => {
    ...
    setSelectedChildId((currentId) => {
      if (
        currentId !== null &&
        children.some((child) => child.id === currentId)
      ) {
        return currentId;
      }
      ...
      if (!initialChildIdAppliedRef.current && initialChildIdParam) {
        ...
        if (
          Number.isInteger(parsedChildId) &&
          children.some((child) => child.id === parsedChildId)
        ) {
  ...
  useEffect(() => {
    ...
    const loadChildProfile = async () => {
      try {
        ...
        setProfileError(
          "Failed to load full child profile. Showing list data.",
        );
      } finally {
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 2 }}>
      <Stack spacing={2}>
        ...
        {childrenLoadError ? (
          <Typography color="error.main">{childrenLoadError}</Typography>
        ) : null}
        {profileError ? (
          <Typography color="error.main">{profileError}</Typography>
        ) : null}
            ...
            <Paper variant="outlined" sx={{ p: 2 }}>
              ...
              >
                ...
                <Box>
                  ...
                  <Typography color="text.secondary">
                    {getAgeLabel(selectedChild.birthDate)} - {groupLabel}
                  </Typography>
            ...
            <Paper variant="outlined" sx={{ p: 2 }}>
              ...
              {!showLoadingIndicator && tab === "profile" ? (
                <Stack spacing={1}>
                  ...
                  <Typography>
                    Birth date:{" "}
                    <strong>{selectedChild.birthDate ?? "Not set"}</strong>
                  </Typography>
                  <Typography>
              ...
              {tab === "development" ? (
                <Typography color="text.secondary">
                  Stage 3 placeholder: milestones and highlights will be added
                  once backend models are ready.
                </Typography>
              ) : null}
            </Paper>
          </Stack>
