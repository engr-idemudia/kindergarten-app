"use client";

import Stack from "@mui/material/Stack";
import Typography from "@mui/material/Typography";
import {
  Button,
  EmptyState,
  Table,
  TableRowActions,
} from "@/src/components/ui";
import type { TableColumn } from "@/src/components/ui";
import { calculateAge } from "@/src/shared/utils/calculateAge";
import { formatPersonName } from "@/src/shared/utils/formatPersonName";
import type { Child } from "../model/child";

type ChildrenTableProps = {
  rows: Child[];
  onViewParentsAction?: (child: Child) => void;
  onManageGroupAction?: (child: Child) => void;
  onViewGroupAction?: (child: Child) => void;
  onEditAction?: (child: Child) => void;
  onDeleteAction?: (child: Child) => void;
};

export default function ChildrenTable({
  rows,
  onViewParentsAction,
  onManageGroupAction,
  onViewGroupAction,
  onEditAction,
  onDeleteAction,
}: ChildrenTableProps) {
  if (rows.length === 0) {
    return (
      <EmptyState
        title="No children found"
        description="Children will appear here after parents start adding them."
      />
    );
  }

  const columns: TableColumn<Child>[] = [
    {
      key: "name",
      label: "Child",
      render: (child) => formatPersonName(child),
    },
    {
      key: "age",
      label: "Age",
      render: (child) => {
        const age = calculateAge(child.birthDate);
        return age === null ? "-" : `${age}`;
      },
    },
    {
      key: "group",
      label: "Group",
      render: (child) =>
        child.group && onViewGroupAction ? (
          <Button
            size="small"
            variant="text"
            onClick={() => onViewGroupAction(child)}
            sx={{ px: 0, minWidth: 0 }}
          >
            {child.group.name}
          </Button>
        ) : (
          (child.group?.name ?? "-")
        ),
    },
    {
      key: "parents",
      label: "Parents",
      render: (child) =>
        onViewParentsAction ? (
          <Button
            size="small"
            variant="text"
            onClick={() => onViewParentsAction(child)}
            sx={{ px: 0, minWidth: 0 }}
          >
            View parents
          </Button>
        ) : (
          <Typography variant="body2">View parents</Typography>
        ),
    },
    {
      key: "actions",
      label: "Actions",
      align: "right",
      render: (child) => (
        <Stack
          direction="row"
          spacing={1}
          justifyContent="flex-end"
          alignItems="center"
        >
          {onManageGroupAction ? (
            <Button
              size="small"
              variant="text"
              onClick={() => onManageGroupAction(child)}
            >
              {child.group ? "Change group" : "Assign group"}
            </Button>
          ) : null}
          <TableRowActions
            onEditAction={onEditAction ? () => onEditAction(child) : undefined}
            onDeleteAction={
              onDeleteAction ? () => onDeleteAction(child) : undefined
            }
          />
        </Stack>
      ),
    },
  ];

  return (
    <Table columns={columns} rows={rows} rowKey={(child) => String(child.id)} />
  );
}
